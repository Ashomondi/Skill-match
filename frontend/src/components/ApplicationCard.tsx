...
export const ApplicationCard = React.memo(({ application }: { application: Application }) => (
  <article className="flex flex-col gap-4 border-b border-[var(--border-hairline)] bg-[var(--bg-secondary)] p-5 last:border-b-0 sm:rounded-lg sm:border">
    <div className="flex items-start gap-3">
      ...
      <span className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${statusStyle[application.status]}`}>{application.status}</span>
    </div>
