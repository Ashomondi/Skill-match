...
export const ResumeUploader: React.FC<ResumeUploaderProps> = ({ onUpload, isUploading = false }) => {
  ...
  const selectFile = async (file?: File) => {
    ...
    if (!ACCEPTED_TYPES.includes(file.type)) { setError('Choose a PDF, DOCX, or TXT file.'); return; }
    if (file.size > MAX_FILE_SIZE) { setError('Your resume must be 10 MB or smaller.'); return; }
    setError(null);
  ...
  return <section className="rounded-lg border border-[#D8C9B2] bg-[#F6F0E6] p-6 shadow-[0px_2px_8px_rgba(92,58,33,0.08)] sm:p-8">
    ...
    <div onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={drop} className={`rounded-lg border border-dashed px-6 py-10 text-center transition sm:px-10 ${isDragging ? 'border-[#5C3A21] bg-[#E3D7C4]/60' : 'border-[#B08D57] bg-[#EFE6D6]/50'}`}>
      <input ref={inputRef} type="file" accept=".pdf,.docx,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain" className="sr-only" onChange={(event) => { void selectFile(event.target.files?.[0]); event.target.value = ''; }} />
      ...
      <p className="mt-4 text-xs text-[#8A7B6B]">PDF, DOCX, or TXT · Maximum file size 10 MB</p>
    </div>
