...
export const Jobs: React.FC = () => {
  ...
  const { jobs, total, totalPages, page, goToPage, loading, error, retry } = useJobs({ query, location });
    ...
    <AppShell>
      <div className="mx-auto w-full max-w-6xl">
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(260px,1fr)_180px_180px_180px]">
          ...
          <select disabled value="" className={`${filterClassName} cursor-not-allowed opacity-60`} aria-label="Filter by seniority (coming soon)" title="Seniority filtering is currently unsupported by the backend"><option value="">Seniority (Coming soon)</option></select>
          <select disabled value="" className={`${filterClassName} cursor-not-allowed opacity-60`} aria-label="Filter by work type (coming soon)" title="Work type filtering is currently unsupported by the backend"><option value="">Work type (Coming soon)</option></select>
        </div>
        ...
        <div className="mt-8 flex items-center justify-between gap-3 text-sm text-[var(--text-muted)]">
          ...
          {(query || location) && <button type="button" onClick={() => { setQuery(''); setLocation(''); }} className="font-semibold text-[var(--text-button-fill)]">Clear filters</button>}
        </div>
    ...
    <AppShell>
      ...
      {loading ? <div className="mt-6 space-y-3" aria-label="Loading job details">{[1, 2, 3].map((item) => <div key={item} className="h-24 animate-pulse rounded-lg border border-[var(--border-hairline)] bg-[var(--bg-card)]" />)}</div> : error ? <div role="alert" className="mt-6 flex flex-col gap-3 rounded-md border border-[var(--status-rejected)] bg-[var(--status-rejected)]/10 p-4 text-sm text-[var(--text-heading)] sm:flex-row sm:items-center sm:justify-between"><span className="flex items-center gap-2"><AlertCircle className="shrink-0 text-[var(--status-rejected)]" size={18} />{error}</span><button type="button" onClick={() => void load()} className="inline-flex items-center justify-center gap-2 rounded-md border border-[var(--text-button-fill)] px-3 py-2 font-semibold text-[var(--text-button-fill)]"><RefreshCw size={15} />Try again</button></div> : job && <div className="mt-5 grid gap-8 lg:grid-cols-[1fr_310px]">
        <article>
          ...
          <section className="mt-8"><h2 className="font-serif text-2xl font-bold text-[var(--text-heading)]">About this role</h2><p className="mt-3 whitespace-pre-wrap leading-7 text-[var(--text-body)]">{formatJobDescription(job.description) || 'No description provided for this role.'}</p></section>
